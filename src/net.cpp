...
static void bleSetup() {
    ...
    bleRxChar = svc->createCharacteristic(NUS_TX_CHAR_RX, NIMBLE_PROPERTY::WRITE | NIMBLE_PROPERTY::WRITE_NR);
...
bool handleSerialCommand(const String &line) {
    ...
    if (line == "ip") {
        logf("ip=%s  mode=%s  rssi=%d",
             ipString().c_str(), ap_mode ? "AP" : "STA", WiFi.RSSI());
        return true;
    }
    if (line == "reboot") {
        ESP.restart();
