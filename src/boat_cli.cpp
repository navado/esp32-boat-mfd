...
void dump_field(const char *name, const Field &f, uint32_t now,
                uint32_t timeout) {
...
bool handleSerialCommand(const String &line) {
    ...
    if (rest.startsWith("timeout ")) {
        ...
        Timeouts t = get_timeouts();
        if      (src == "n2k")  t.nmea2000_ms = ms;
        else if (src == "wifi") t.nmea_wifi_ms = ms;
        else if (src == "sk")   t.signalk_ms = ms;
        else if (src == "demo") t.demo_ms = ms;
        else {
    ...
    if (rest.length() == 0 || rest == "snapshot") {
        ...
        dump_field("heading_true_rad", s.heading_true_rad,    now, timeout_for(t, s.heading_true_rad.source));
