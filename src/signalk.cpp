...
bool handleSerialCommand(const String &line) {
    ...
    if (line == "sk-status") {
        net::logf("host=%s port=%u connected=%d lastUpdateAgo=%lums",
                  s_host.c_str(), s_port, data.connected,
                  data.lastUpdateMs ? (millis() - data.lastUpdateMs) : 0);
        return true;
    }
    if (line == "sk-dump") {
        net::logf("lat=%.5f lon=%.5f", data.lat, data.lon);
        net::logf("sog=%.2f m/s (%.1f kn)  cog=%.3f rad  hdg=%.3f rad",
                  data.sog, isnan(data.sog) ? 0.0 : data.sog * 1.94384,
                  data.cogTrue, data.headingTrue);
        net::logf("aws=%.2f m/s awa=%.3f rad  tws=%.2f twa=%.3f",
                  data.aws, data.awa, data.tws, data.twa);
        net::logf("depth=%.2fm  water=%.2fK  batt=%.2fV soc=%.2f",
                  data.depth, data.waterTemp, data.battVoltage, data.battSoc);
        return true;
    }
    return false;
}
