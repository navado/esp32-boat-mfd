...
static Arduino_DataBus *bus = new Arduino_SWSPI(
    -1 /* DC */, ST7701_CS, ST7701_SCK, ST7701_MOSI, -1 /* MISO */);
...
static Arduino_ESP32RGBPanel *rgbpanel = new Arduino_ESP32RGBPanel(
    ...
    1 /* pclk_active_neg */, 12000000L /* prefer_speed 12MHz */, false /* useBigEndian */);
...
static void touch_read_cb(lv_indev_t *indev, lv_indev_data_t *data) {
    ...
    Wire.write(0x81); Wire.write(0x4E);  // GT911 status register
...
static void ui_refresh(lv_timer_t *) {
    ...
    char buf[64];
    if (!isnan(d.aws)) { snprintf(buf, sizeof(buf), "%.1f kn", d.aws * 1.94384); lv_label_set_text(lbl_aws, buf); }
    if (!isnan(d.awa)) {
    ...
    if (!isnan(d.sog)) { snprintf(buf, sizeof(buf), "%.1f", d.sog * 1.94384); lv_label_set_text(lbl_sog, buf); }
    if (!isnan(d.cogTrue)) {
    ...
    if (!isnan(d.depth)) { snprintf(buf, sizeof(buf), "%.1f", d.depth); lv_label_set_text(lbl_depth, buf); }
...
void setup() {
    ...
    lv_display_set_buffers(disp, buf_a, buf_b,
