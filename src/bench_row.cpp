...
int BenchRow::header(char *out, size_t cap) {
    return snprintf(out, cap,
                    "screen,mode,fps,flush_avg_us,flush_peak_us,refresh_us,lvgl_peak_us,"
...
int BenchRow::format(const char *label, char *out, size_t cap) const {
    ...
    return snprintf(out, cap,
                    "%s,%.1f,%.0f,%.0f,%.0f,%.0f,%.0f,%.0f,%.0f,%.1f,%.1f,%.0f,%.0f,%.1f,%.0f,%.0f,"
                    "%.0f,%.0f,%.0f,%.0f,%.1f",
                    label ? label : "", r.fps, r.flush_avg_us, r.flush_peak_us, r.refresh_us,
                    r.lvgl_peak_us, r.core0_idle_pct, r.core1_idle_pct, r.loop_peak_us, r.deltas_s,
                    r.parsed_s, r.parse_avg_us, r.store_size, r.store_lookups_s, r.ws_frames_s,
                    r.ws_bytes_s, r.subscriptions, r.heap_lowwater_kb, r.largest_block_kb,
                    r.min_stack_b, r.temp_c);
}
